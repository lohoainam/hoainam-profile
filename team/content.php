...
if(trim($_POST['text'])){
	extract($_POST);
	$name = trim($name);
	if(!$name || $name != $_SESSION['user_login']) die('Không chat dc nhá, đăng nhập đã tề');
...
$scontent = $scontent ? fetch_emoticon($scontent) : 'Chào mừng các bạn đến với chat room của website Hoài Nam Dev, bây giờ thì dùng tự nhiên nào :)), nhớ ủng hộ web nhé!';
echo $scontent;
?>
<br /><br />
